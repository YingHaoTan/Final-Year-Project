...
import org.powertac.samplebroker.core.interfaces.PortfolioManager;
	...
	private void createInitialTariffs() {
		...
		for (PowerType pt : customerProfiles.keySet()) {
			...
			if (pt.isStorage()) {
				...
				rr.withUpRegulationPayment(rateValue * 1.2).withDownRegulationPayment(-rateValue * 0.4); // magic
																											// numbers
				spec.addRate(rr);
			}
			spec.addRate(rate);
		...
		if (371 == timeslotIndex) {
			for (TariffSpecification spec : tariffRepo.findTariffSpecificationsByBroker(brokerContext.getBroker())) {
				if (PowerType.INTERRUPTIBLE_CONSUMPTION == spec.getPowerType()) {
					BalancingOrder order = new BalancingOrder(brokerContext.getBroker(), spec, 0.5,
					...
					BalancingOrder bup = new BalancingOrder(brokerContext.getBroker(), spec, 1.0, up);
					BalancingOrder bdown = new BalancingOrder(brokerContext.getBroker(), spec, -1.0, down);
